...
class KBucket:
    def __init__(self, range_lower, range_upper, ksize, replacementNodeFactor=5):
...
class TableTraverser:
    def __init__(self, table, startNode: Node):
...
class RoutingTable:
    ...
    def add_contact(self, node: Node):
        ...
        if bucket.has_in_range(self.node) or bucket.depth() % 5 != 0:
            ...
            # in original, we'd ping head; here, we ignore.
            pass
    ...
    def find_neighbors(self, node: Node, k=None, exclude=None):
        k = k or self.ksize
        nodes = []
